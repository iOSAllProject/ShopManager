...
- (id)initWithNibName:(NSString *)nibNameOrNil bundle:(NSBundle *)nibBundleOrNil
{
    ...
    if (self) {
        ...
        self.title = NSLocalizedString(@"productViewController.title", nil);
...
- (void)viewDidLoad
{
    ...
    UIBarButtonItem *btnAddProduct = [[UIBarButtonItem alloc] initWithTitle:NSLocalizedString(@"productViewController.button-add", nil) style:UIBarButtonItemStylePlain target:self action:@selector(addProduct)];
    self.navigationItem.rightBarButtonItem = btnAddProduct;
}
...
-(void)valueChanged
{
    ...
    [scroller setContentSize:CGSizeMake(320, 529)];
    ...
    for(int i=0;i<[allFeaturedArray count];i++)
    {
        ...
        if([[[productInfo objectForKey:@"general"] objectForKey:@"product_type"] isEqualToString:@"simple"] || [[[productInfo objectForKey:@"general"] objectForKey:@"product_type"] isEqualToString:@"external"])
        {
            ...
            [self fullImageBox:[[productInfo objectForKey:@"product_gallery"] objectForKey:@"featured_images"]
                  ...
                  regularPrice:[AppDelegate convertToThousandSeparator:[[[productInfo objectForKey:@"general"] objectForKey:@"pricing"] objectForKey:@"regular_price"]]
                     salePrice:[AppDelegate convertToThousandSeparator:[[[productInfo objectForKey:@"general"] objectForKey:@"pricing"] objectForKey:@"sale_price"]]
